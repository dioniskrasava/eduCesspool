...
/*- - - - - - - -    В Ы В Е Д Е Н И Е     Т И П А     - - - - - - - -  */


/** Kotlin автоматически может понять, что за тип у пеменной исходя из значения
 * которым мы инициализируем данную переменную
 *
 * */
fun typeInference(){
    ...
    val height = 1.81       // Double
    val height2 = 1.80f     // Float


    // НЕЛЬЗЯ ОБЪЯВИТЬ ПЕРЕМЕННУЮ БЕЗ ТИПА А ПОЗЖЕ ИНИЦИАЛИЗИРОВАТЬ
    /*
     * val age // ERROR
     * age = 5
     */
...
/*- - - - - - - -    П Р И В Е Д Е Н И Е     Т И П А      - - - - - - - -  */
fun typeConversion(){
    /*
    * Присвоить значение одного типа переменной другого типа данных напрямую нельзя
    *
    * поэтому их явно нужно преобразовывать
    * */
...
fun typeAny(){
    /*
    * Есть такой супертип, которому можно присвоить любое значение
    * */
